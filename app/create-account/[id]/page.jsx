...
const CreateAccount = () => {
    ...
    async function handleGoogleLogin(e){
      e.preventDefault()
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/google/redirect`,{
        method: 'GET',
        headers: {
          'Api-Key': `${process.env.NEXT_PUBLIC_API_KEY}`,
        }
        ...
        <div className="flex flex-col justify-center items-center relative z-[11]">
          <div className="bg-white px-4 sm:px-8 pt-8 pb-5 rounded-[16px] border w-full sm:w-[588px]">
            ...
            <div className="text-center mb-12 mt-[-80px] relative z-[100]">
              ...
              <p className="text-[#667085] sm:text-[14px] text-[14px]">Dive back into a world of premium features and explore the endless possibilities that await. Reconnect and continue your journey today.</p>
            </div>
            
            <form className="flex flex-col sm:w-[400px] mx-auto">

              <div className="w-full flex flex-col gap-3 mb-[20px]">
                <button onClick={handleGoogleLogin} className="border border-[#D0D5DD] text-[#ffffff] py-2 px-4 rounded-[8px] sm:text-[16px] text-[14px] flex items-center justify-center w-full shadow">
              ...
              <div>
                <label htmlFor="email" className="text-[#ffffff] gont-[500] text-[14px] mb-1 block">Email</label>
                <input
                  type="email"
              ...
              <div className='my-5'>
                <label className="text-[#ffffff] gont-[500] text-[14px] mb-1 block">Password</label>
                  ...
                  <div>
                    {
                      ...
                      <ImCheckboxUnchecked className='cursor-pointer text-gray-400' onClick={() => setPasswordType('text')}/>
                      :
                      <ImCheckboxChecked className='cursor-pointer text-gray-400' onClick={() => setPasswordType('password')}/>
                    }
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="text-[#ffffff] gont-[500] text-[14px] mb-1 block">Confirm Password</label>
                <div className='flex items-center justify-between border border-gray-300 p-2 rounded-[6px] w-full'>
                <input
                    type={passwordType}
              ...
              <div className='mt-5'>
                <label htmlFor="referral-code" className="text-[#ffffff] gont-[500] text-[14px] mb-1 block">Referral Code</label>
...

              {
                ...
                <BtnLoader />
                :
                <button onClick={handleAccountCreation} className="bg-primary-color text-white py-2 px-4 rounded-[8px] mt-5 text-[14px]">
                  Create Account
                </button>
              }
            </form>
            <div className="text-[white] text-[14px] flex items-center gap-1 mt-2 justify-center">
              ...
              <Link href="#" className="inline-flex items-center gap-[2px] text-primary-color">Privacy Policy <FiExternalLink/></Link> 
              and 
              <Link href="#" className="inline-flex items-center gap-[2px] text-primary-color">Terms of Use <FiExternalLink/></Link>
            </div>

            <div className="text-center text-[black] mt-3 text-[14px]">
              Already have an account? <Link href="/login" className="text-primary-color">Log in</Link>
            </div>
          </div>
        </div>
      </div>
