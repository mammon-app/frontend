...
const PasswordReset = () => {
    ...
    async function resendOtp(e) {
        e.preventDefault()
        setLoading(true)
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/resend-forgot-password-otp`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Api-Key': `${process.env.NEXT_PUBLIC_API_KEY}`,
                ...
                <div className=" flex flex-col justify-center items-center relative z-[11]">
                    <div className="bg-white px-4 sm:px-8 pt-8 pb-5 rounded-[16px] border w-full sm:w-[588px]">
                        ...
                        <div className="text-center mb-12 mt-[-80px] relative z-[100]">
                            ...
                            <p className="text-[#667085] text-[14px] mt-3">Please input the OTP sent to {email}</p>
                        </div>
                        <div className="flex flex-col sm:w-[400px] mx-auto">

                            <div>
                                <label htmlFor="otp" className="text-[#ffffff] gont-[500] text-[14px] mb-1 block">OTP</label>
                                <input
                                    type="text"
                            ...
                            <p className='text-[white] cursor-pointer text-center mt-2 text-[14px]' onClick={resendOtp}>Click to resend code?</p>
                            {
                                loading ?
                                    <BtnLoader />
                                    :
